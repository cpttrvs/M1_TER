...
males-own [tail-size energie energie-depart]
femelles-own [energie energie-depart gestation? largest-tail-size largest-size remaining father-size father-tail]
...
to femelles::Mate
  let actu ioda:my-target
  set largest-size [size] of actu
  set largest-tail-size [tail-size] of actu
  set gestation? true
  set remaining 15
  set father-tail [tail-size] of actu
  set father-size [size] of actu
end
...
to giveBirth
  ifelse random 4 < 3
    ...
    set shape "butterfly"
    set size [father-size] of myself + 1
    setxy random-xcor random-ycor

    set tail-size [father-tail] of myself + 1
    set energie-depart starting-energy + random (size * 10)
    set energie energie-depart
    set color scale-color blue tail-size 0 max-tail-size
    set energie-depart starting-energy + random (size * 10)
...
to males::filter-neighbors
  ioda:filter-neighbors-in-cone 2 60
  ioda:add-neighbors-on-links my-links
end

to femelles::filter-neighbors
  ioda:filter-neighbors-in-cone vision angle
  ioda:add-neighbors-on-links my-links
end
;;;conditionels

to-report males::exhausted?
  report energie < 4
end

to-report femelles::IFoundBetter?
  run ChooseMethod
end

to-report BetterSize?
  let actu ioda:my-target
  report [size] of actu > largest-size
end

to-report BetterTail?
  let actu ioda:my-target
  report [tail-size] of actu > largest-tail-size
end

to-report femelles::Gestation?
  report gestation?
end

to-report males::dead?
  report energie <= 0
end

to-report femelles::dead?
  report energie <= 0
end
...
to create-breed-male
    set shape "butterfly"
    setxy random-xcor random-ycor
    set energie max-lives * energie-depart
...
"" ""
PENS
"default" 1.0 0 -14454117 true "" "plot count males"
"pen-1" 1.0 0 -2064490 true "" "plot count femelles"
