...
to dieYouFool
  ask turtles with [distance myself < 5 and side = [side] of myself]
    [enrage]
  die
end
...
to hit [x]
  if(hit? x) [
    ifelse(not block? x) [ask x [set life (life - (maxNumber 1 (([strenght] of myself) - armor)))]]
    [set blocked true]
  ]
end
...
to randomAct
  ifelse(life < healLimit * totalLife)
   [set life (life + 1)]
   [moveAtRandom]
end
...
to seeYourLife [x]
  ifelse (life < (totalLife * fleeLimit) / wrath)
  [flee x]
  [attack x]
end

to act
  let x min-one-of (turtles in-cone 20 60) with [side != [side] of myself] [distance myself]
  ifelse(x != nobody) [
    seeYourLife x
    ]
    [
      let y min-one-of (turtles in-radius 5) with [side != [side] of myself] [distance myself]
      ifelse(x != nobody)[seeYourLife y]
      [randomAct]
    ]
end
...
Polygon -7500403 true true 15 45 15 105 45 75 15 45 30 75 30 90 45 75 45 75 15 45 60 45 45 75 105 195 90 210 75 180
...
Polygon -7500403 true true 75 165 75 165 45 120 15 75 30 135 60 180
