...
humans-own [ groupId originalColor camp
  strength startingStrength life energy
  parry hasParried? nextTickParry
  desertKnowledge forestKnowledge mountainKnowledge fieldKnowledge
  target
  d:s
  inTrench? tickTrench]

patches-own [ value base environmentType effectValue alteration ]

globals [ timerUpdate
  averageStrength averageStrengthBlue averageStrengthRed
  averageParry averageParryBlue averageParryRed

  default desert desertColor forest forestColor mountain mountainColor lake lakeColor ]
...
to attack
  ...
  ifelse any? other humans with [groupId != myGroupId] in-radius vision [
    ...
    ifelse one-of other humans-here with [groupId != myGroupId] != nobody [
      ...
      initInteraction ;; prépare les cognitons pour l'intéraction
...
to initHuman [group humanColor]
  ...
  ask one-of patches with [environmentType != 4] [
    ...
    ;;gestion de la parade (valeur aléatoire en fonction de la variance)
    ...
    set nextTickParry false

    ;;gestion de la connaissance de l'environnement
...
to wiggle
  rt random 70
  lt random 70
  if [environmentType] of patch-ahead 1 != lake [
    fd 0.5
  ]
end
...
to go_trench
  ifelse inTrench?
  [ ifelse tickTrench < max-trench-time
    [ set tickTrench tickTrench + 1 ]
    [ set inTrench? false
      wiggle ] ]
  [ ask patch-ahead 1 [ modifyEnvironment 1 ]
    fd 1
    set inTrench? true
    set tickTrench 0 ]
end
...
to monitorPlots
  ifelse timerUpdate > update-frequency [
    ;;average strength
    set averageStrength 0
    set averageStrengthBlue 0
    set averageStrengthRed 0
    ;;average parry
    set averageParry 0
    set averageParryBlue 0
    set averageParryRed 0

    ask humans [
      set averageStrength averageStrength + strength
      set averageParry averageParry + parry
      if color = red [
        set averageStrengthRed averageStrengthRed + strength
        set averageParryRed averageParryRed + parry
      ]
      if color = blue [
        set averageStrengthBlue averageStrengthBlue + strength
        set averageParryBlue averageParryBlue + parry
      ]
    ]

    set averageStrength averageStrength / (count humans)
    set averageParry averageParry / (count humans)

    if count humans with [color = blue] > 0
    [
      set averageStrengthBlue averageStrengthBlue / (count humans with [color = blue])
      set averageParryBlue averageParryBlue / (count humans with [color = blue])
    ]

    if count humans with [color = red] > 0
    [
      set averageStrengthRed averageStrengthRed / (count humans with [color = red])
      set averageParryRed averageParryRed / (count humans with [color = red])
    ]
